...
# MAGIC 
# MAGIC The following CTAS statement creates a simple, unpartitioned external Delta Lake table from a sample dataset 파티션되지 않은 기본 external 테이블 생성 
...
# MAGIC 1. Avoid unnecessary storage costs (note significant differences in bytes for <a href="https://spark.apache.org/docs/latest/sql-ref-datatypes.html" target="_blank">numeric types</a>
# MAGIC 
# MAGIC ### Datetime Filtering
# MAGIC If data will be frequently filtered by year, year & month, day of week, date, or another datetime value, consider calculating these values at write time if not present in original data. (Pushdown filters work best on fields present in a table). 날짜단위 
# MAGIC 
...
# MAGIC Fields that will be used for filtering and optimizations should appear at the beginning of the schema declaration. 앞에 32개만 통계정보 모으니 중요 필드들은 테이블 정의시 앞으로
...
# MAGIC 
# MAGIC **NOTE**: These statistics are generally uninformative for string fields with very high cardinality (such as free text fields). 텍스트필드/high cardinality 데이터에 대해서는 통계정보 무의미 , 텍스트 컬럼들은 뒤쪽으로 정리하게.. You can omit these fields from statistic collection by <a href="https://docs.databricks.com/delta/optimizations/file-mgmt.html#data-skipping" target="_blank">moving them outside the first 32 columns or changing the number of columns on which statistics are collected</a>.
...
# MAGIC Z-order나 bloom filter하려면 통계정보가 있어야한다.
...
# MAGIC 
# MAGIC Delta Lake에서의 파티션 방법은 Hive나 Parquet기반의 Spark에서의 그것과 유사합니다. 
# MAGIC 
...
display(spark.read.json(f"{DA.paths.working_dir}/date_part_table/_delta_log/00000000000000000001.json"))
